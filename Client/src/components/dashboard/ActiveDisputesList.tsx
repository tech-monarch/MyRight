    ...
    <div>
      ...
      {disputes.length > 0 ? (
        <>
          ...
          <div className="hidden md:block border border-(--color-border-light) divide-y divide-(--color-border-light)">
            {disputes.map((dispute, index) => {
              const status = statusConfig[dispute.status];
              return (
                <div
                  key={dispute.id}
                  className="flex flex-row items-center justify-between gap-4 p-5 bg-white hover:bg-(--color-bg-off-white) transition-colors duration-150 group"
                >
          ...
          <div className="md:hidden space-y-4">
            {disputes.map((dispute, index) => {
              ...
              return (
                <div
                  key={dispute.id}
                  className="bg-white border border-(--color-border-light) rounded-lg p-4 shadow-sm"
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-(--color-text-muted) uppercase tracking-wider">
                        <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${status.dot}`} />
                        {status.label}
                      </span>
                      <span className="text-[10px] font-bold text-(--color-text-muted) bg-(--color-bg-off-white) px-2 py-0.5 uppercase tracking-wider border border-(--color-border-light)">
                    ...
                    <div className="text-xs text-gray-400">{dispute.id.slice(0, 8)}</div>
                  </div>
                  <h3 className="text-sm font-bold text-(--color-primary-navy) mb-1">
                    {dispute.title}
                  </h3>
                  <p className="text-[11px] text-(--color-text-muted) mb-4">
                    Initiated {dispute.dateInitiated}
                  </p>
                  <button
                    onClick={() => navigate(`/dispute/${dispute.id}/chat`)}
                    className="w-full flex items-center justify-center gap-1.5 text-xs font-bold uppercase tracking-wider bg-(--color-primary-navy) text-white px-4 py-2 rounded-md hover:bg-(--color-primary-blue) transition-colors duration-150"
                  >
                    <RiMessage3Line className="w-3.5 h-3.5" />
                    Chat with your Case
                  </button>
        ...
        <div className="border border-dashed border-(--color-border-light) p-16 flex flex-col items-center justify-center text-center bg-white">
          ...
          <p className="text-xs text-(--color-text-muted) max-w-xs mb-6 leading-relaxed">
            You have no ongoing cases. Start the resolution process and let MyRight guide you.
          </p>
          <Link
            to="/dispute/new"
            onClick={() => resetChat()}
